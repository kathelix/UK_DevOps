...
- `make-processing` is excluded by the query but never set, as in Make. `make-failed` is also excluded; unlike Make, the script *does* set it to keep one poison message from blocking the queue — on a per-message **read-side** processing failure (e.g. an HTML decode error) **and** on a **write-side** deterministic Airtable reject (a `4xx` isolated to one record when its sub-batch siblings wrote fine). A transient write failure (`429`/`5xx`) is never `make-failed` — it just retries next run — and a systemic reject (every record `4xx`) quarantines nothing and fails loud. Setting `make-processing`, and retry-count handling for the still-uncovered repeatedly-transient case, remain `TODO.md` items.
