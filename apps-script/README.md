...
- **Same query, same labels** as the original Make scenario (`-label:job-vacancies/make-collected …`, adds `make-collected` on success). The script shared label state with the Make scenario so the two could run side by side during the parallel-run transition without double-collecting; Make was **decommissioned 2026-06-17**, so the script is now the sole collector. Unread status is never touched (as in the original Make scenario).
