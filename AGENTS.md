...
- **Branch = work type, declared by the Architect in the slice spec.** A `<!-- branch: chore/x -->` comment names the branch outright; `<!-- type: fix -->` shorthand → `fix/<slice>`. The prefix encodes the work type (`feature/`, `fix/`, `chore/`, `docs/`, `refactor/`), defaulting to `feature/<slice>` when unset. `scripts/dispatch-slice.sh` resolves it (CLI arg > `branch:` > `type:` > default) and `git check-ref-format`–validates the result; see `docs/SLICE_DISPATCH.md`.
