...
- [ ] **`make-failed` on a repeatedly-transient write / set `make-processing`.** `make-processing` is never set (as in Make). `make-failed` _is_ already set for **two** poison cases: a per-message read-side processing exception (decode/parse error → see the catch block in `collectJobEmailsLocked_`) **and** a write-side **deterministic** Airtable reject (a `4xx` isolated to one record via the sub-batch individual-retry path — PR #19). Still uncovered: a message that fetches fine and whose write fails **transiently** (`429`/`5xx`) on every run is left to retry forever by design (a transient is never quarantined). Add retry-count-based `make-failed` labelling (label after N transient failures) to cap that, and decide whether `make-processing` is worth setting. _The cross-run counter was rejected for the deterministic case (slower, doesn't unblock siblings — `docs/TECH_DESIGN.md` §2); a counter still fits the repeatedly-transient case._
