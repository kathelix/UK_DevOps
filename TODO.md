...
- [x] **Regression test for the cleaning regex** using `tests/fixtures/email.html` (e.g. clasp + local runner, or an in-project `checkFixture()` assertion function). _(Done 2026-06-08, branch `collector/reliability-net`: a no-dependency `node --test` harness in `/tests` loads the `.gs` in a `vm` sandbox — golden regression on the real fixture, plus `parseFrom_` / `decodeB64Url_` and the reliability-slice helpers; runs in CI via `.github/workflows/tests.yml`.)_
...
- [ ] **Modularize for testability** — split into `config / gmail / parser / airtable / main`; keep cleaning, link-extraction and dedupe as pure functions (no Gmail/Airtable side effects) so they run against `tests/fixtures/` locally. _(Partial 2026-06-08: `buildUpsertPayload_` and `isOverRuntimeBudget_` extracted as pure helpers and unit-tested; the full module split is still pending.)_
...
- [x] **Timeout safety** — track elapsed time, stop cleanly before the Apps Script execution limit, leave the rest for the next run (pairs with the `MAX_MESSAGES` batch size). _(Done 2026-06-08, branch `collector/reliability-net`: `MAX_RUNTIME_MS` break in the fetch loop and, after review F1, the write/label loop. Reworked 2026-06-09 into one budget guard per sub-batch — see the forward-progress item below; exercised by `tests/collect-loop.test.js`.)_
- [x] **Write-phase forward-progress guard.** The write/label timeout break deferred the whole run if the fetch loop alone exhausted `MAX_RUNTIME_MS` (the guard tripped at `i=0`, writing nothing); under sustained slow-fetch latency this re-collected the same head-of-queue messages with no forward progress. _(Done 2026-06-09, branch `collector/sub-batch-pipeline`: replaced the two-phase fetch-all-then-write-all structure with an interleaved sub-batch pipeline — `CONFIG.SUB_BATCH_SIZE=5`, fetch → upsert → label per sub-batch, committed before the next. The first sub-batch effectively always runs, so every run commits ≥1 sub-batch and a timeout/crash loses at most one. Guarded by the forward-progress test in `tests/collect-loop.test.js`.)_
