...
- [x] **Timeout safety** — track elapsed time, stop cleanly before the Apps Script execution limit, leave the rest for the next run (pairs with the `MAX_MESSAGES` batch size). _(Done 2026-06-08, branch `collector/reliability-net`: `MAX_RUNTIME_MS` break in the fetch loop and, after review F1, the write/label loop; both exercised by `tests/collect-loop.test.js`.)_
- [ ] **Write-phase forward-progress guard.** The write/label timeout break defers the whole run if the fetch loop alone exhausts `MAX_RUNTIME_MS` (the guard trips at `i=0`, writes nothing); under sustained slow-fetch latency this re-collects the same head-of-queue messages with no forward progress. Either flush ≥1 batch before the guard can break, or confirm defer-all is acceptable. Idempotent retries protect data, not liveness.
