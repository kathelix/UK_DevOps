...
- [ ] **Map `nexxt.com`'s footer (pending its own capture + parity pass).** The PII bar holds (a single `sent by Nexxt, c/o Nexxt Inc` text marker leaves no recipient email post-cut in **both** the `alert@` postal-first and `jfw@` action-first templates, byte-confirmed 2026-06-20). The `get_thread` QP-decode that blocked a faithful fixture is now handled by the **`fixture-raw-transport`** transport (parity-gated raw-RFC822 capture, shipped for `talent.com` 2026-06-24); `nexxt.com` just needs its own capture + per-message parity pass via it (marker `text`; `jfw@` is effectively n=1 — don't pin a 1-sample marker). Best re-staged as the Architect's own follow-up slice. See `docs/TECH_DESIGN.md` §4 (the `nexxt.com` deferral note).
