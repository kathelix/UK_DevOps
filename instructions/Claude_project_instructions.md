...
- **Freshness & topic rotation (stateful).** The post's *lead* must be a fresh angle, not the day's dominant noise — enforced against a persisted history (Airtable **PostTopics**), not left to memory. SC-clearance fatigue is real — the LinkedIn audience is bored of it.
  ...
  4. **At run end** (alongside the §9 done-marker writes): write one PostTopics row `{ Date: today, Topic: <lead topic phrase, ≤ ~40 chars> }` (`create_records_for_table`), then **trim** PostTopics to the most recent ~14 rows (delete older via `delete_records_for_table`) so it stays a bounded rolling window, not an inventory (the per-base 1,000-record cap — `docs/TECH_DESIGN.md` §5). The trim is a **required step** — always attempted, never skipped as a feature. But, like the run-start read, the write **and** trim are a **non-gating** freshness feeder: if either fails (table absent, name mismatch, missing delete permission, an Airtable hiccup), **report it in the batch report and continue** — **never** fail the run, suppress the Post, or block the §9 done-marker writes. A skipped trim self-corrects next run (growth is ~1 row/day; the window re-bounds; OPERATIONS flags a PostTopics far past ~14 rows). This keeps the degrade-clean contract symmetric across read and write.
