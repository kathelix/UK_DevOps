...
- **Freshness & topic rotation (stateful).** The post's *lead* must be a fresh angle, not the day's dominant noise — enforced against a persisted history (Airtable **PostTopics**), not left to memory. SC-clearance fatigue is real — the LinkedIn audience is bored of it. **PostTopics is the Post's own rotation state: screening, the `<date>_recommend-flag.md` handoff, and the §9 done-marker NEVER depend on it — they always run, and a PostTopics failure never halts, re-screens, or un-marks them. Every PostTopics failure is scoped to the Post/rotation feature and always fails *loud* (a prominent ❌ in the batch report), never silent.**
  1. **At run start** (alongside the §0 Vacancies read): read **PostTopics** rows where `Date` ≥ today − 7 days; hold their `Topic` values as the **suppressed set** (≤ ~7). PostTopics lives in the **same base** as §0/§1 (`appV9puNHinuRKTk9`); resolve it **by name** (`list_records_for_table`) — a deliberate exception to the §0/§1 "explicit IDs" rule, acceptable because it runs **once per run**, not in the hot per-email path. **An empty result is normal** (day 1, or a genuinely quiet 7-day window) → use an empty suppressed set and proceed. **A read *error*** (table missing/renamed, permission denied, a PostTopics-specific Airtable error — reachable only when the §0 Vacancies read **succeeded**, i.e. the base is up; a base-wide outage already alert-and-stops upstream at §0/§1) means a fresh lead can't be guaranteed → **skip Post generation** (the Post **and** the image-concepts deliverable) and lead the batch report with a prominent ❌ (e.g. *"PostTopics unreadable — Post skipped today; fix the table"*). **Screening, the recommend/flag handoff, and §9 proceed normally** (per the independence rule above) — do **not** proceed with an empty set on an *error* (that would silently mask a broken rotation store and ship a possibly-stale lead).
  ...
  4. **At run end** (after the Post is produced, alongside the §9 done-marker writes): write one PostTopics row `{ Date: today, Topic: <lead topic phrase, ≤ ~40 chars> }` (`create_records_for_table`), then **trim** PostTopics to the most recent ~14 rows (delete older via `delete_records_for_table`) so it stays a bounded rolling window, not an inventory (the per-base 1,000-record cap — `docs/TECH_DESIGN.md` §5). The trim is the **bounded-growth safeguard** and is **required every run**. **A write or trim/cleanup *error* fails loud but does not discard the Post or block screening:** the Post was already built from a valid suppressed set, so it is **fresh** — **deliver it**, and surface a prominent ❌ naming the failed op (**write** → *"today's topic not recorded; tomorrow's rotation may repeat it"*; **trim** → *"bounded-growth safeguard skipped — watch the per-base record cap"*). **Never** discard a good Post or silently defer the failure to "next run" (no guarantee the trim ever runs; PostTopics could grow into the record cap). The §9 done-marker `Status` flips are **independent** and proceed regardless (PostTopics is rotation state, not a dedup marker) — a PostTopics failure never re-screens or un-marks the batch.
  5. **Starvation fallback.** If *every* candidate lead today is already in the suppressed set (a genuinely repetitive day), do **not** emit a stale lead and do **not** skip the post: lead with the **least-recently-used** candidate topic and note in the batch report that rotation fell back. **LRU = oldest *most-recent* use, not oldest row:** a topic can have several rows in the window (a prior fallback re-led a suppressed topic), so compute each candidate's `lastSeen = max(Date)` across its rows and pick the candidate whose `lastSeen` is **oldest** — never just the oldest row, which could belong to a topic also used yesterday. (Granular topic phrases make this rare.)
