...
- **Freshness & topic rotation (stateful).** The post's *lead* must be a fresh angle, not the day's dominant noise — enforced against a persisted history (Airtable **PostTopics**), not left to memory. SC-clearance fatigue is real — the LinkedIn audience is bored of it.
  ...
  4. **At run end** (after the Post is produced, alongside the §9 done-marker writes): write one PostTopics row `{ Date: today, Topic: <lead topic phrase, ≤ ~40 chars> }` (`create_records_for_table`), then **trim** PostTopics to the most recent ~14 rows (delete older via `delete_records_for_table`) so it stays a bounded rolling window, not an inventory (the per-base 1,000-record cap — `docs/TECH_DESIGN.md` §5). The trim is the **bounded-growth safeguard** and is **required every run**. **A write or trim *error* is fail-loud:** report it as a **failed run** (lead the batch report with a prominent ❌ naming the failed PostTopics op) so it is fixed immediately — **never** swallow it or defer to "next run", which has no guarantee the trim ever runs and lets PostTopics grow into the record cap. The §9 done-marker `Status` flips are an **independent** dedup marker and are **not** rolled back by a PostTopics failure (PostTopics is rotation state, not a dedup marker) — so a run-end PostTopics error fails *loud* without re-screening or un-marking the batch.
