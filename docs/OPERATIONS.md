...
|---|---|---|
| Collector run red in Executions | Airtable API change/outage, expired PAT, or any transient (`429`/`5xx`) sub-batch upsert failure (fail-loudly is by design) | Read execution log; messages stay uncollected and retry next run — no data loss by design (write-then-label ordering) |
