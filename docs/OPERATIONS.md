...
**Token resource-access — do this first, both cases.** A restore creates a **new base id**, and the project's PATs are scoped *per base* (see *Secrets inventory* — both are "Job Search base **only**"). Airtable requires the new base to be **added as a resource** on a PAT before that token can touch it, or the call fails `403 INVALID_PERMISSIONS_OR_MODEL_NOT_FOUND` ([PAT resource-access contract](https://support.airtable.com/docs/creating-personal-access-tokens)). So: grant the new base to the GAS data PAT **`AIRTABLE_TOKEN`** before the first collector/backup run (**both** cases); and in case B, grant it to **`AIRTABLE_SCHEMA_TOKEN`** *before* running `apply-schema.js`/`import-schema.js`. If you mint **replacement** PATs instead of editing resource access, update the GitHub secret (`AIRTABLE_SCHEMA_TOKEN`) and the GAS Script Property (`AIRTABLE_TOKEN`) accordingly. **Verify both PATs against the new base before declaring restore complete.**

**A. Airtable native snapshot (in-platform, same account).** A snapshot **preserves the table and field IDs**, so only the **`baseId`** changes. Restore the snapshot to a new base, grant the new base to `AIRTABLE_TOKEN` (token note above), then repoint the base id only: `git grep appV9puNHinuRKTk9` and update every hit — `instructions/Claude_project_instructions.md` (§0/§1, the §9 done-marker flip, the *Applied & Skipped roles* section), the collector `CONFIG.AIRTABLE_BASE_ID` **and** `BACKUP.AIRTABLE_BASE_ID` in `apps-script/vacancies-backup.gs`, and `airtable/schema.json` — then redeploy. (Use this when available — it is the simple case.)
