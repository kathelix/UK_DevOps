...
- **Upsert failures end the run Failed (fail-loudly).** A transient Airtable failure — `429`/`5xx`/network throw — leaves the **whole** sub-batch unlabelled to retry next run; a deterministic `4xx` is re-sent record-by-record so healthy siblings still commit (see *Failed message* above). Either way, if any record is left uncollected the execution ends **Failed** (`N sub-batch upsert(s) failed; first: …`) so the GAS failure email fires — previously a hard write-block (e.g. at the record cap) stalled RawEmails silently while every run showed "Completed". A red collector run with this message means *some* messages weren't written; they are not lost. **Systemic guard:** if **every** record in a sub-batch is rejected `4xx` (no healthy sibling — bad auth, wrong endpoint, or schema drift), the run quarantines **nothing** and fails loud, so a deploy mistake can't mass-`make-failed` the queue; fix the systemic cause and the next run clears it.
...
|---|---|---|
| Collector run red in Executions | Airtable API change/outage, or any transient sub-batch upsert failure — `429`/`5xx` or a network transport throw (`network error: …`); a **missing** `AIRTABLE_TOKEN` fails fast with `Script property AIRTABLE_TOKEN is not set` (fail-loudly is by design) | Read execution log; messages stay uncollected and retry next run — no data loss by design (write-then-label ordering) |
