...
domain and proposes a ready-to-paste candidate marker `'<domain>': '<phrase>'`. *(A third class —
**residual** — is a mapped sender the collector **did** cut but whose cut left an **earlier**
footer element/tracker (the mapped marker wasn't the earliest; e.g. an **array** sender). The §3
scan distinguishes it from drift by how much remains (a whole uncut footer = drift; only an earlier
fragment, bulk already cut = residual) and §8 proposes **appending** an earlier marker to the array
— **not** replacing the working one. See step 3 / `docs/TECH_DESIGN.md` §4.)*
...
3. Add (new sender) or correct (drift) the `'<domain>': '<phrase>'` entry in `FOOTER_MARKERS`
   ...
   NIJobs digests; §3 confirms the mapped marker still cuts the sender's other rows), make the
   domain's value an **array** and **append** the earlier marker — do **not** replace the working
   one, which still serves the other template (earliest valid cut wins; `docs/TECH_DESIGN.md` §4).
   For a **new** sender, add a **registered-domain**
