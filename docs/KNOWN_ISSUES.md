...
**Residual:** rename-safety applies only to entries that carry an id. RawEmails and Vacancies carry live ids in `schema.json` (RawEmails backfilled 2026-06-17, PR #36; Vacancies already did), so their fields are rename-safe. The footgun is **live for any newly added** table/field committed name-only — until `import-schema.js` is run once (`schema.bases:read`) to backfill its id. The **PostTopics** table (added 2026-06-27 for §8 post-topic rotation) is exactly such an entry: committed **name-only**, so until import-schema backfills its `tbl…`/`fld…` ids, treat `schema.json` as the authority on its names and **don't rename its `Date`/`Topic` fields in the UI** (a UI rename would re-create them as duplicates on the next apply).
