...
**Residual:** rename-safety applies only to entries that carry an id. Both managed tables now carry live ids in `schema.json` (RawEmails backfilled 2026-06-17, PR #36; Vacancies already did), so every current managed field is rename-safe. The footgun returns only for a **newly added** table/field committed name-only — until `import-schema.js` is run once (`schema.bases:read`) to backfill its id. For any such not-yet-backfilled entry, treat `schema.json` as the authority on names and don't rename it in the UI.
